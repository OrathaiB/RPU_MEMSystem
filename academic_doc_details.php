...
// ฟังก์ชันอัพเดตสถานะเป็น "จัดพิมพ์แล้ว"
function updatePrintStatus($conn, $id) {
    $sql = "UPDATE exam_requests SET status_doc = 'จัดพิมพ์แล้ว' WHERE id = ?";
...
// ตรวจสอบการส่งฟอร์ม
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['print_completed'])) {
    if (updatePrintStatus($conn, $id)) {
        echo "<script>alert('อัพเดตสถานะเป็น \"จัดพิมพ์แล้ว\" เรียบร้อย'); window.location.href = 'academic_approved_list.php';</script>";
        exit;
    } else {
        echo "<script>alert('เกิดข้อผิดพลาดในการอัพเดตสถานะ');</script>";
    }
}
        ...
        <div class="form-container">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <!-- เพิ่มปุ่ม "ดำเนินการจัดพิมพ์เรียบร้อย" -->
                    <form id="printCompletedForm" method="POST" action="">
                        <center><button type="button" onclick="confirmPrintCompleted()"
                                class="btn btn-success btn-lg btn-block">
                                ดำเนินการจัดพิมพ์เรียบร้อย
                            </button></center>
                        <input type="hidden" name="print_completed" value="1">
                    </form>
            ...
            function confirmPrintCompleted() {
                if (confirm("ยืนยันการบันทึกข้อมูล?\nสถานะเอกสารจะถูกอัพเดตเป็น \"จัดพิมพ์แล้ว\"")) {
                    document.getElementById('printCompletedForm').submit();
                    return true;
                }
                return false;
            }
