...
function approveDocument($conn, $id) {
    $sql = "UPDATE exam_requests SET status_doc = 'อนุมัติแล้ว', approved_date = NOW() WHERE id = ?";
...
if (isset($_POST['approve'])) {
    if (approveDocument($conn, $id)) {
        echo "<script>alert('อนุมัติเอกสารเรียบร้อยแล้ว'); window.location.href = 'waitapproved_list.php';</script>";
        exit;
    } else {
        echo "<script>alert('เกิดข้อผิดพลาดในการอนุมัติเอกสาร');</script>";
    }
}

// ประมวลผลการไม่อนุมัติ
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['reject_submit'])) {
    $reason = $_POST['reject_reason'];
    $status = 'ไม่อนุมัติ';
    
    $sql = "UPDATE exam_requests SET status_doc = ?, reject_reason = ?, reject_date = NOW() WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssi", $status, $reason, $id);
    
    if ($stmt->execute()) {
        echo "<script>alert('บันทึกการไม่อนุมัติเรียบร้อยแล้ว'); window.location.href = 'document_list.php';</script>";
        exit;
    } else {
        ...
        <div class="form-container">
            ...
            <div class="row mb-3">
                <div class="col d-flex justify-content-between">
                    ...
                    <?php if ($document['status_doc'] !== 'อนุมัติแล้ว' && $document['status_doc'] !== 'จัดพิมพ์แล้ว'): ?>
                    <a href="editform.php?id=<?php echo $id; ?>" class="btn btn-warning">แก้ไขแบบฟอร์ม</a>
                    <?php endif; ?>
                </div>
            </div>
            ...
            ?>
                    ...
                    <p><strong>แบบฟอร์มอื่นๆ:</strong><br>
                        <?php echo htmlspecialchars($document['other_form']); ?> <br>จำนวน:
                        <?php echo htmlspecialchars($document['other_form_count']); ?> แผ่น<br>
                    ...
                    <p><strong>ไฟล์ข้อสอบ:</strong><br>
                        <?php echo htmlspecialchars($document['file_name']); ?> <br>
                        <i class="fas fa-file-alt document-icon ml-4"
            ...
            <div class="d-flex justify-content-center mt-4">
                ...
                <button type="button" class="btn btn-danger ms-3" data-bs-toggle="modal" data-bs-target="#rejectModal">
                    ไม่อนุมัติ
                </button>
            </div>
            <?php endif; ?>
            ...
            <!-- Modal สำหรับกรณีไม่อนุมัติ -->
            <div class="modal fade" id="rejectModal" tabindex="-1" aria-labelledby="rejectModalLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="rejectModalLabel">กรุณาระบุเหตุผลที่ไม่อนุมัติ</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form method="POST">
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="reject_reason" class="form-label">เหตุผลที่ไม่อนุมัติ:</label>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" name="reject_submit" class="btn btn-primary">ยืนยัน</button>
                            </div>
                        </form>
                    </div>
                </div>
